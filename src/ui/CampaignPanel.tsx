...
function missionDescription(campaign: Campaign): string {
  const missionId = getCampaignState(campaign.id).currentMission;
  if (!missionId) {return '';}
  return t(`campaigns:${campaign.id}.missions.${missionId}.description`);
}
...
const CampaignEntry: Component<{ campaign: Campaign }> = (props) => {
  return (
    <div class="campaign-entry campaign-active">
      ...
      <div class="campaign-mission-hint">{missionDescription(props.campaign)}</div>
    </div>
  );
};
